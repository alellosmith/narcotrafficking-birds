# This script maps the overlap between increased narcotrafficking suitability and IBLs for migratory birds
...
# read in migratory forest birds important polygons
polygons_fd_mig <- read_sf("data/final/important_polygons_forest-dependent_migrants.gpkg")

# intersect - note that sf is intelligent with attribute data!
overlap_increase <- st_intersection(polygons_fd_mig, narco_suit_combined) 

overlap_increase %>%
  st_geometry() %>%
  write_sf("data/final/overlap_nt_suitability_mig_forest_birds.gpkg")

# write out simplified version (without lines or points) for migration connections map
types <- st_geometry_type(overlap_increase)
overlap_increase_nolines <- overlap_increase[types != "MULTILINESTRING", ]
overlap_increase_ex <- st_collection_extract(overlap_increase_nolines, "POLYGON") %>%
  write_sf("migration_connections_inputs/polygons/overlap_nt_suitability_mig_forest_birds.gpkg")



# now, plot both areas of nt suitability increase and bird importance, and intersection of the layers
# plot -----
png("plots/1_overlap_ibls_migrant_forest_birds_nt_increase.png", width = 1500, height = 1500)
par(mar = c(2, 0.25, 4, 0.25))
# plot country borders
plot(central_america, lwd = 4)
# plot important bird areas
plot(st_geometry(polygons_fd_mig), col = alpha("blue", alpha = 0.5), 
     border = NA, add = TRUE)
...
#       cex.main = 4, line = -1)
# 
# legend(x = "bottom", legend=c("IBLs for migratory forest birds", "Areas of increased suitability for narco-trafficking",
...
area_fd_mig_ibl <- polygons_fd_mig %>% 
  mutate(area = st_area(.) %>% units::set_units("km2") %>% as.numeric()) %>% 
  as_tibble() %>% 
  summarize(area_total = sum(area))
# total area of 30% most important areas for migratory landbirds = 106,003 km^2

# identify which bird group and shipment level these stats are for, then save to data/intermediate
area_stats <- area_intersect %>%
  cbind(area_fd_mig_ibl) %>%
  mutate(percent_overlap = area_overlap/area_total) %>%
  mutate(id = "primary_mig_forest_birds") %>%
  write_csv("data/intermediate/forest-migrant_stats.csv")
