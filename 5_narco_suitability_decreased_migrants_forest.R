# This script maps the overlap between decreased narcotrafficking suitability and IBLs for migratory birds
...
overlap_decrease <- st_intersection(polygons_fd_mig, multipol_suit_decreased) 

overlap_decrease %>%
  st_geometry() %>%
  write_sf("data/final/overlap_suitability_decrease_mig_forest_birds.gpkg")

# now, plot both areas of nt suitability decrease and bird importance, and intersection of the layers
# plot -----
png("plots/2_overlap_ibls_migrant_forest_birds_nt_decrease.png", width = 1500, height = 1500)
par(mar = c(2, 0.25, 4, 0.25))
# plot country borders
plot(central_america, col = NA, lwd = 4)
...
#       cex.main = 4, line = -1)
# 
# legend(x = "bottom", legend=c("IBLs for migratory forest birds", "Areas of decreased suitability for narco-trafficking",
...
area_stats <- area_intersect %>%
  ...
  mutate(id = "mig_forest_birds_decrease") %>%
  write_csv("data/intermediate/forest-migrant_stats_nt_decrease.csv")
